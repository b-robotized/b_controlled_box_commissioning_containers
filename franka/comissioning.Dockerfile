...
COPY franka.jazzy.repos .
RUN --mount=type=ssh \
  vcs import . < franka.jazzy.repos
# clones a single named dir from a github repo - only fetches the workspace directory for this specific robot.
RUN clone_robot_dir "https://github.com/b-robotized/b_ctrldbox_commissioning.git" "franka" "b_ctrldbox_commissioning/"
